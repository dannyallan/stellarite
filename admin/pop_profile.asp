...
<%
	...
	strTitle = Application("IDS_UserProfile")
	...
	If strDoAction <> "" Then
		...
		ElseIf strDoAction = "edit" Then
			...
			If not (objRS.BOF and objRS.EOF) Then
				If lngRecordId <> CLng(objRS.fields(0).value) Then Call sendBack(Application("IDS_UserExists"))
			End If
		...
		ElseIf strDoAction = "new" Then
			...
			Set objRS = objConn.Execute(getUserIdSql(strUserName))
			If not (objRS.BOF and objRS.EOF) Then	Call sendBack(Application("IDS_UserExists"))
	...
	Else
		...
		If blnRS Then
			...
			If not (objRS.BOF and objRS.EOF) then
				strFirstName = showString(objRS.fields("U_FirstName").value)
				strLastName = showString(objRS.fields("U_LastName").value)
				strAddress1 = showString(objRS.fields("U_Address1").value)
				strAddress2 = showString(objRS.fields("U_Address2").value)
				strAddress3 = showString(objRS.fields("U_Address3").value)
				strCity = showString(objRS.fields("U_City").value)
				strState = showString(objRS.fields("U_State").value)
				strCountry = showString(objRS.fields("U_Country").value)
				strZIP = showString(objRS.fields("U_ZIP").value)
				intTimeZone = objRS.fields("U_TimeZone").value
				strEmail = showString(objRS.fields("U_Email").value)
				lngPhone1 = showPhone(objRS.fields("U_Phone1").value)
				intExt1 = objRS.fields("U_Ext1").value
				lngPhone2 = showPhone(objRS.fields("U_Phone2").value)
				intExt2 = objRS.fields("U_Ext2").value
				strUserName = showString(objRS.fields("U_UserName").value)
			End If
		Else
			intTimeZone = -5
		End If
...
<script language="Javascript">
	function checkPwd() {
		...
		if (pass != document.forms[0].txtConPassword.value) {
			alert('<% =Application("IDS_MsgPasswordMatch") %>');
...
<div id="contentDiv" class="dvBorder" style="height:420px;"><br>

<table border=0 width="100%">
<form name="frmProfile" method="post" action="pop_profile.asp?id=<% =lngRecordId %>">
<% =getHidden("hdnAction","") %>
<% =getHidden("hdnChange","") %>
<% =getHidden("hdnWinOpen","") %>
    <tr>
      <td><% =getLabel(Application("IDS_Name"),"strFirstName") %></td>
      <td>
    ...
    <tr>
      ...
      <td class="dfont"><% =getStates(140,"selState",strState) %></td>
    </tr>
    <tr>
      <td><% =getLabel(Application("IDS_Country"),"selCountry") %></td>
      <td><% =getCountries(140,"selCountrry",strCountry) %>
      &nbsp;&nbsp;&nbsp;&nbsp;
      <% =getLabel(Application("IDS_ZIP"),"txtZip") & getTextField("txtZIP","oText",strZIP,7,7,"") %></td>
    </tr>
    <tr>
      <td><% =getLabel(Application("IDS_TimeZone"),"selTimeZone") %></td>
    ...
    <tr>
      <td><% =getLabel(Application("IDS_UserName"),"txtUserName") %></td>
      <td><% =getTextField("txtUserName","mText",strUserName,20,20,"") %></td>
    </tr>
    <tr>
      <td><% =getLabel(Application("IDS_Password"),"txtNewPassword") %></td>
...
<%
	If blnRS and blnAdmin Then
		Response.Write(getIconNew("pop_profile.asp"))
