...
<%
	...
	strTitle = Application("IDS_CRMParameters")
	...
	If strDoAction = "edit" Then
		...
		Call setConfigValue("av_CRMURL",strURL)
		Call setConfigValue("av_TimeZone",intTimeZone)
		Call setConfigValue("av_Navigation",blnNavigation)
		Call setConfigValue("av_Currency",strCurrency)
		Call setConfigValue("av_TaxRate",decTaxRate)
		Call setConfigValue("av_TaxChange",blnTaxChange)
		Call setConfigValue("av_LoginAttempts",bytLoginAttempts)
		Call setConfigValue("av_MaxRecords",intMaxRecords)
		Call setConfigValue("av_HideUsers",blnHideUsers)
...
%>
<div id="contentDiv" class="dvBorder" style="height:350px;"><br>

<table border=0 cellspacing=5 width="100%">
<form name="frmAdmin" method="post" action="pop_parameters.asp">
<% =getHidden("hdnAction","") %>
<% =getHidden("hdnChange","") %>
<% =getHidden("hdnWinOpen","") %>
  <tr>
    <td><% =getLabel(Application("IDS_CRMURL"),"txtURL") %></td>
  ...
  <tr>
    <td><% =getLabel(Application("IDS_Navigation"),"selNavigation") %></td>
    <td>
      <select name="selNavigation" id="selNavigation" class="oBool" onChange="doChange();" style="width:260px;">
        <option value="0"<% =getDefault(0,blnNavigation,"0") & ">" & Application("IDS_NavIcons") %></option>
        <option value="1"<% =getDefault(0,blnNavigation,"1") & ">" & Application("IDS_NavButtons") %></option>
      </select>
   </td>
  </tr>
  <tr>
    <td><% =getLabel(Application("IDS_TaxRate"),"txtTaxRate") %></td>
    <td><% =getTextField("txtTaxRate","mCurrency",decTaxRate,4,4,"") %></td>
  </tr>
  <tr>
    <td><% =getLabel(Application("IDS_LoginAttempts"),"txtLoginAttempts") %></td>
    <td><% =getTextField("txtLoginAttempts","mByte",bytLoginAttempts,4,4,"") %></td>
  </tr>
  <tr>
    <td><% =getLabel(Application("IDS_MaximumRecords"),"txtMaxRecords") %></td>
    <td><% =getTextField("txtMaxRecords","mInt",intMaxRecords,4,6,"") %></td>
  </tr>
  <tr>
    <td colspan="2"><hr><% =getCheckbox("chkTaxChange",blnTaxChange,"") %>&nbsp;&nbsp;&nbsp;<% =getLabel(Application("IDS_MsgTaxChange"),"chkTaxChange") %></td>
  </tr>
  <tr>
    <td colspan="2"><% =getCheckbox("chkHideUsers",blnHideUsers,"") %>&nbsp;&nbsp;&nbsp;<% =getLabel(Application("IDS_MsgHideUsers"),"chkHideUsers") %></td>
  </tr>
</form>
</table>
