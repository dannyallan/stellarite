...
<%
	...
	strTitle = getIDS("IDS_Event")

	If strDoAction <> "" then

		Select Case strDoAction
			Case "del"
				If intPerm >= 4 Then Call delEvent(lngUserId,lngRecordId,bytMod,lngModId)
		...
		If bytMenu = 0 Then Session("LastPage") = strCRMURL & "common/i_events.asp?m=" & bytMod & "&mid=" & lngModId
		Call closeEdit()
	Else
		...
		If blnRS and intPerm >= 3 Then
			...
			If not (objRS.BOF and objRS.EOF) then
				...
				strEventTitle = objRS.fields("E_Title").value
		...
		Else
			...
			datModDate = Now
		End If
		strEventType = getOptionDropDown(150,False,"selEventType","IDS_EventType",intEventType)
	End If

	Call DisplayHeader(0)
	Call showEditHeader(strTitle,strCreatedBy,datCreatedDate,strModBy,datModDate)
%>
<form name="frmEvent" method="post" action="edit_event.asp?id=<% =lngRecordId %>&m=<% =bytMod %>&mid=<% =lngModId %>&menu=<% =bytMenu %>">
...
<% =getHidden("hdnChange","") %>
  <tr>
	<td><% =getLabel(getIDS("IDS_Title"),"txtTitle") %></td>
  ...
  <tr>
	<td><% =getLabel(getIDS("IDS_Start"),"selSMonth") %></td>
	<td colspan=3>
  ...
  <tr>
	<td><% =getLabel(getIDS("IDS_End"),"selEMonth") %></td>
	<td colspan=3>
  ...
  <tr>
	<td><% =getLabel(getIDS("IDS_Type"),"selEventType") %></td>
  ...
  <tr>
	<td><% =getLabel(getIDS("IDS_Billable"),"chkBillable") %></td>
	<td colspan=3><% =getCheckbox("chkBillable",blnBillable,"") %></td>
  </tr>
  <tr>
	<td><% =getLabel(getIDS("IDS_Onsite"),"chkOnsite") %></td>
	<td colspan=3><% =getCheckbox("chkOnsite",blnOnsite,"") %></td>
 </tr>
<%	Call editCustomFields(50) %>
</table>
